...
namespace OneTradeCentral.iOS
{
	public partial class OrderViewController : UIViewController
	{
		...
		public string SelectedProductUOM { get; set; }
		...
		static string selectedUOM { get; set; }
		...
		public override void ViewDidLoad ()
		{
			...
			// EmailAddressTableView = new UITableView(View.Bounds);
...
//					// Uncommented by Eldon for Testing
			...
			StoreMgrEmailField.EditingDidEnd += (object sender, EventArgs e) => {
				...
				// persist the changes to the customer record
				this.CustomerEmails = StoreMgrEmailField.Text;
...
//				ContactLookupButton.Enabled = false;
		...
		partial void addNewEmailAddress(UIButton sender)
		{
			UIAlertView alert = new UIAlertView();
			string _newEmailAddress;
			alert.Title = "Email Address";
			alert.AddButton("Add");
			alert.AddButton("Cancel");
			alert.Message = "Key-in additional email address:";
			...
			alert.Clicked += (object s, UIButtonEventArgs ev) =>
			{
				if (ev.ButtonIndex == 0)
				{
					_newEmailAddress = alert.GetTextField(0).Text;

					if (this.CustomerEmails.IndexOf(_newEmailAddress) < 0 )
					{
						if (this.CustomerEmails.Trim() == "")
						{
							this.CustomerEmails += _newEmailAddress.Trim();
						}
						else
						{
							this.CustomerEmails += "," + _newEmailAddress.Trim();
						...
						string[] _storeEmailAddresses = this.CustomerEmails.Split(',');
						EmailAddressTableView.Source = new EmailAddressTableSource(_storeEmailAddresses, this);
						EmailAddressTableView.ReloadData();
						Order.Customer.ContactEmail = Order.StoreMgrEmail = StoreMgrEmailField.Text = this.CustomerEmails;
					}
		...
		public void SetStoreMgrFields (string firstname, string lastname, string email, string phone)
		{
			...
			// Populate the email addresses and put them to the table view

			this.CustomerEmails = email;
			string[] _storeEmailAddresses = email.Split(',');
			EmailAddressTableView.Source = new EmailAddressTableSource(_storeEmailAddresses, this);
		...
		public void AddOrderLineItem (Product product, int quantity)
		{
			...
			orderDetail.ProductUOM = product.UOM; 
			...
			if (productLineItem == null) { 
				...
				orderDetail.ProductUOM = product.UOM;
				productLineItem.ProductUOM = product.UOM;
			}
...
//			}
		...
		private bool IsValidEmail {
			get {
				...
				_emailAddresses = this.CustomerEmails.Split(','); // StoreMgrEmailField.Text.Trim().Split(',');

				for (int indx = 0; indx < _emailAddresses.Length; indx++)
				{
					if (Regex.IsMatch(_emailAddresses[indx], @"\A(?:[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)\Z") == false)
					{
						_countInvalid++;
					}
				}

				return (_countInvalid == 0 && _emailAddresses.Length > 0);
		...
		public override void PrepareForSegue (UIStoryboardSegue segue, NSObject sender)
		{
			...
			CoreGraphics.CGSize pWarehouseControllerSize = new CoreGraphics.CGSize (450, 216);
			...
			case "OrderConfirmationSegue":
				...
				this.Order.PONumber = this.PONumberField.Text;

				Order.Customer.ContactEmail = Order.StoreMgrEmail = this.CustomerEmails;
			...
			case "WarehouseSelectionSegue":
				...
				warehouseController.PreferredContentSize = pWarehouseControllerSize;
			...
			case "EditOrderLineSegue":
				...
				if (orderEditorLineController != null) {
					...
					// Get the current ordeline and pass the UOM here
					orderEditorLineController.SelectedProductUOM =  selectedUOM;
...
//				break;
		...
		public class OrderDetailSource : UITableViewSource
		{
			...
			public override void RowSelected (UITableView tableView, NSIndexPath indexPath)
			{
				...
				selectedUOM = _orderLineItems [indexPath.Row].ProductUOM;
...
//				cell.DetailTextLabel.Text = _orderLineItems [indexPath.Row].Quantity.ToString ();
				...
				cell.DetailTextLabel.Text = String.Format ("Code: {0}, UOM: {1}, Qty: {2}", orderLine.Product.Code, orderLine.ProductUOM,
				                                           orderLine.Quantity);
				return cell;
			}
