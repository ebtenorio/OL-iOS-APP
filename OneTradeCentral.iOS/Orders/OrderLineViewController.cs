	...
	public partial class OrderLineViewController : UIViewController
	{
		...
		public Product SelectedProduct { 
			...
			set{
				...
				if (UOMButton != null)
					UOMButton.Enabled = true;

				//if (_selectedProduct.UOM != null && _selectedProduct.UOM.Trim () != "") {
				if (UOMTextField != null) {
					if (SelectedProductUOM != null) {
						UOMTextField.Text = SelectedProductUOM;
					} else {
						UOMTextField.Text = _selectedProduct.UOM;
					}
		...
		public override void ViewDidLoad ()
		{
			...
			this.UOMButton.Enabled = false;

			DALFacade dal = new DALFacade ();

			// Check here if there is an existing UOM, else default to the first one.
		// Get a look

//			if (dal.getUOMList ().Count > 0) {
//				this.UOMTextField.Text = dal.getUOMList () [0].Name;
//			}

			Provider provider = new Provider ();

			provider = dal.getProviderByID (OrderController.ProviderID);

			if (provider.IsPepsiDistributor == true) {
				this.UOMButton.Hidden = false;
				this.UOMTextField.Hidden = false;
				this.UOMLabel.Hidden = false;
			} else {
				this.UOMButton.Hidden = true;
				this.UOMTextField.Hidden = true;
				this.UOMLabel.Hidden = true;
			}
		...
		partial void addItem (Foundation.NSObject sender)
		{
			...
			if (quantityText != null && quantityText.Trim().Length > 0) {
				try {
					...
					var uom = this.UOMTextField.Text;

					if(uom.Trim() != ""){
						_selectedProduct.UOM = uom;
					}
...
//					Logger.log ("Cannot convert quantity to text: " + e.Message);
		...
		public override void PrepareForSegue (UIStoryboardSegue segue, NSObject sender)
		{
			...
			} else if (segue.Identifier == "UOMSelectionSegue") {
				var uomSelectionViewController = segue.DestinationViewController as UOMViewController;
				uomSelectionViewController.OrderLineController = this;
