...
//				sourceController.DismissViewController (true, null);
		...
		class OrderListSource : UITableViewSource
		{
			...
			public override UITableViewCell GetCell (UITableView tableView, NSIndexPath indexPath)
			{
				...
				_provider = dalFacade.getProviderByID (order.ProviderID);
				...
				if (order.UploadStatus == (int) Order.STATUS.Completed) {
					...
					if (_provider.IsPepsiDistributor == true) {
						if (order.PONumber.Trim () == "") {
							poNumberOrStatus = "Order No: " + order.OrderNumber;
						} else {
							poNumberOrStatus = "PO No: " + order.PONumber + ", Order No: " + order.OrderNumber;
						}
					} else {
					...
					if (_provider.IsPepsiDistributor == true) {
						if (order.PONumber.Trim () == "") {
							poNumberOrStatus = "Order No: " + ((Order.STATUS)order.UploadStatus).ToString ();
						} else {
							poNumberOrStatus = "PO No: " + order.PONumber + ", Order No: " + ((Order.STATUS)order.UploadStatus).ToString ();
						}
					} else {
