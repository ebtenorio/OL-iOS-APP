...
//					"select distinct GroupName as Name from [ProductGroupLookup] where ProductName like ?", productName);
		...
		public IList<Provider> getProviderListByCustomerIDFiltered (long customerID)
		{
			...
			var queryProvider = db.Query<Provider> (
				                    "select distinct p.* from [Provider] p join [Customer] c on p.ID = c.ProviderID where p.Inactive = 0 and p.Deleted = 0 "
				                    + "and c.ID = ? and c.StartDate <= ? and c.EndDate >= ? Order By p.Name ASC", customerID, DateTime.Now, DateTime.Now);
			...
			var queryProviderWarehouse = db.Query<ProviderWarehouse> (
				                             ...
				                             + "and pw.StartDate <= ? and pw.EndDate >= ? Order By pw.Name Asc", DateTime.Now, DateTime.Now);
...
//				return new List<Provider> ();
		...
		public void saveProductUOM(UOM productUOM) {
			if (productUOM.PK <= 0)
				db.Insert (productUOM);
			else
				db.Update (productUOM);
		}

		public IList<UOM> getProductUOMList (){
			var query = db.Table<UOM> ().OrderBy(t => t.Name);
			if (query != null && query.Count() > 0)
				return query.ToList();
			else
				return new List<UOM>();
		...
		{
			var query = db.Table<ProviderWarehouse>().Where ( t => t.Deleted == false && t.InActive == false &&
				t.ProviderID == providerID && t.StartDate <= DateTime.Now && t.EndDate >= DateTime.Now).OrderBy(t => t.Name);
			if (query != null && query.Count() > 0)
				return query.ToList();
			else
				return new List<ProviderWarehouse>();
