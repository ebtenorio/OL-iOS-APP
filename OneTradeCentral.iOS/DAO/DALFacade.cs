...
//			var warehouseList = ws.downloadWarehouseList ();
			...
			if (providerRows != null && providerRows.Length > 0) {
				for (int i = 0; i < providerRows.Length; i++) {
					...
					if (providerTable.Columns.Contains ("IsPepsiDistributor")) {
						if (providerRows [i] ["IsPepsiDistributor"] != System.DBNull.Value)
							provider.IsPepsiDistributor = (bool)providerRows [i] ["IsPepsiDistributor"];
					}
			...
			// Additional UOMS - This should not affect the existing customers/clients

			if (referenceData.Tables ["tblProductUOM"] != null) {
				DataTable productUOMTable = referenceData.Tables ["tblProductUOM"];
				DataRow[] productUOMRows = productUOMTable.Select ();
				if (productUOMRows != null && productUOMRows.Length > 0) {
					for (int i = 0; i < productUOMRows.Length; i++) {
						UOM productUOM = new UOM ();
						productUOM.Code = productUOMRows [i] ["ProductUOM"].ToString ();
						productUOM.Name = productUOMRows [i] ["ProductUOM"].ToString ();
						dao.saveProductUOM (productUOM);
					}
				}
			}
